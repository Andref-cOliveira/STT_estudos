...
index2byte = ['<EMP>', ' ', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 
              'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'ã', 'á', 'à', 'â', 'ç', 'é', 'ê',
              'í', 'õ', 'ô', 'ú', 'ü']
...
def _load_mfcc(src_list):
    ...
    encoding = 'utf-8'
    # load mfcc
    mfcc = np.load(str(mfcc_file, encoding), allow_pickle=False)
...
class SpeechCorpus(object):
    ...
    def __init__(self, batch_size=16, set_name='train'):
        ...
        #for ind, lab in enumerate(label):
            #print(np.fromstring(lab,dtype=int))
        # to constant tensor
